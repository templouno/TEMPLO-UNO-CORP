...
const emptyForm = { name: '', phone: '', email: '', cpf_cnpj: '', city: '', state: '', notes: '', company_id: '' };
...
export default function ClientsPage() {
  ...
  const loadClients = useCallback(() => {
    setLoading(true);
    api.clients.list(search).then(setClients).finally(() => setLoading(false));
  }, [search]);
  ...
  const openEdit = (c: Client) => { setEditing(c); setForm({ name: c.name, phone: c.phone||'', email: c.email||'', cpf_cnpj: c.cpf_cnpj||'', city: c.city||'', state: c.state||'', notes: c.notes||'', company_id: c.company_id||'' }); setDialogOpen(true); };

  const handleSave = async () => {
    if (!form.name) return;
    setSaving(true);
    try {
      if (editing) await api.clients.update(editing.id, form);
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-zinc-500">{clients.length} cadastrados</p>
        </div>
      ...
      <div className="relative mb-4">
        ...
        <Input className="pl-9" placeholder="Buscar clientes..." value={search}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-zinc-800 bg-zinc-900">
                {['Nome','Contato','Cidade','Empresa','Pedidos','Total Gasto',''].map(h => (
                  <th key={h} className="px-4 py-3 text-left text-xs font-medium text-zinc-500">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-800/50">
              {clients.map((c: Client & { company_name?: string; company_color?: string }) => (
                <tr key={c.id} className="bg-zinc-900 hover:bg-zinc-800/50 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="flex h-7 w-7 items-center justify-center rounded-full bg-zinc-800 text-xs font-medium text-zinc-300 flex-shrink-0">
                  ...
                  <td className="px-4 py-3">
                    {c.city && <div className="flex items-center gap-1 text-xs text-zinc-400"><MapPin className="h-3 w-3" />{c.city}{c.state ? `, ${c.state}` : ''}</div>}
                  </td>
                  <td className="px-4 py-3">
                    {c.company_name && (
                      <span className="inline-flex items-center gap-1.5 text-xs text-zinc-400">
                        <div className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: c.company_color }} />
                        {c.company_name}
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => handleDelete(c.id, c.name)} className="rounded p-1.5 text-zinc-500 hover:bg-red-500/20 hover:text-red-400 transition-colors"><Trash2 className="h-3.5 w-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} className="max-w-xl">
        <DialogHeader onClose={() => setDialogOpen(false)}>{editing ? 'Editar Cliente' : 'Novo Cliente'}</DialogHeader>
