...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const order = db.prepare(`
    SELECT o.*, cl.name as client_name, cl.phone as client_phone, cl.email as client_email,
      cl.city as client_city, cl.state as client_state, cl.cpf_cnpj as client_cpf_cnpj,
  ...
  const fullOrder = { ...order, products, checklist, payments };

  // Cast to satisfy @react-pdf/renderer's strict element type
  const element = React.createElement(OrderPDFDocument, { order: fullOrder as never }) as unknown as React.ReactElement<DocumentProps>;
  ...
  return new NextResponse(buffer as unknown as BodyInit, {
    headers: {
      ...
      'Content-Disposition': `inline; filename="pedido-${order.number}.pdf"`,
    },
  });
}
